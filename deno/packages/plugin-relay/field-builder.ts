...
fieldBuilderProto.node = function node({ id, ...options }) {
    return this.field<{}, InterfaceRef<unknown>, unknown, Promise<unknown>, true>({
        ...
        resolve: async (parent: unknown, args: {}, context: object, info: GraphQLResolveInfo) => {
            const rawID = (await id(parent, args as never, context, info)) as GlobalIDShape<SchemaTypes> | string | null | undefined;
            if (rawID == null) {
                return null;
            }
...
fieldBuilderProto.connection = function connection({ type, ...fieldOptions }, { name: connectionNameFromOptions, ...connectionOptions }, { name: edgeNameFromOptions, ...edgeOptions }) {
    const placeholderRef = this.builder.objectRef<ConnectionShape<unknown, false>>("Unnamed connection");
    ...
    this.builder.configStore.onFieldUse(fieldRef, (fieldConfig) => {
        ...
        const connectionFields = (connectionOptions.fields as unknown) as ObjectFieldsShape<SchemaTypes, ConnectionShape<unknown, false>> | undefined;
