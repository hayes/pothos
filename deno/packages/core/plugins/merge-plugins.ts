import { GraphQLFieldResolver, GraphQLSchema, GraphQLTypeResolver } from 'https://cdn.skypack.dev/graphql@v15.5.0?dts';
...
export class MergedPlugins<Types extends SchemaTypes> extends BasePlugin<Types> {
    ...
    onInputFieldConfig(fieldConfig: GiraphQLInputFieldConfig<Types>) {
        return this.plugins.reduceRight((config, plugin) => plugin.onInputFieldConfig(config), fieldConfig);
    }
    onOutputFieldConfig(fieldConfig: GiraphQLOutputFieldConfig<Types>) {
        return this.plugins.reduceRight((config, plugin) => plugin.onOutputFieldConfig(config), fieldConfig);
    }
    onEnumValueConfig(valueConfig: GiraphQLEnumValueConfig<Types>) {
        return this.plugins.reduceRight((config, plugin) => plugin.onEnumValueConfig(config), valueConfig);
    }
    beforeBuild() {
