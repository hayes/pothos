...
The plugin supplies one complete include plan. Prisma chooses its SQL execution
strategy. Nested relations can execute in one statement; the number of statements
depends on the Collection, target capabilities, and upstream planner.
...
`query` accepts either a literal `{ where, orderBy, limit, offset }` or a
...
use `t.relationCount` and `t.relationAggregate`. Both compile
...
Aggregate operations and compatible fields come from the emitted contract's
`AggregateTypes`. `count` defaults to GraphQL Int; other numeric reducers default
to Float, so fractional sum/min/max results are not truncated or rejected as Int.
Pass `type` to choose a compatible scalar. Non-number results require it:

```ts
largeCount: t.relationAggregate('posts', { op: 'countBigInt', type: 'BigInt' }),
smallestTitle: t.relationAggregate('posts', {
  op: 'min', field: 'title', type: 'String',
}),
```

Register custom scalars such as `BigInt` with their matching Pothos input/output
types. Available operations vary with the contract, not Pothos dialect branches:
for example PostgreSQL contracts can provide `avgDecimal`. `count`/`countBigInt`
may also receive a field to count its non-null values. Use a lossless operation
and scalar when values could exceed GraphQL Int or JavaScript number precision.
...
With `prismaNext.collections` configured, a model row reached through a regular
`t.field` can load missing selections when it carries its primary or unique key.
The loader batches requests across parents. Prefer `t.prismaField` for root
collections so the plugin can preload compatible fields together. Without a
provider, relation data must already be present; otherwise the resolver reports
that the relation was not loaded.

## Independent to-one refinements

Compatible selections of the same to-one relation share an include. Prisma ORM
RC9 rejects `combine()` on to-one includes (`ORM.INCLUDE_UNSUPPORTED`). Configure
`prismaNext.collections` to handle incompatible filters: the initial query loads
one compatible group, and a multi-row fallback query loads each remaining group
with its own nested selections. Aliases receive independent values, including
filtered `null` results. Without a provider, incompatible refinements are rejected
before SQL. Independent to-many consumers continue to use combine slots.
