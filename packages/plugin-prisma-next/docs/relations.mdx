...
Compatible selections of the same to-one relation are merged. Prisma ORM rc9
rejects `combine()` on to-one includes (`ORM.INCLUDE_UNSUPPORTED`), so two aliases
requiring incompatible filters cannot currently share the complete include plan.
The plugin rejects that plan before executing SQL. Independently filtered to-many
aliases remain supported. This is an upstream composition limit, not a reason
to load unselected relations later through fallback resolvers.
