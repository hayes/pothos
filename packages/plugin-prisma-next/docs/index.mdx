...
<Callout type="warn">
  Initial 0.1 release targeting Prisma ORM `8.0.0-rc.9` (formerly Prisma Next).
  Pin the upstream packages to this exact release candidate. The plugin uses
  the shared SQL family and has no database-dialect configuration; MongoDB's
  separate Collection API is not supported.
</Callout>

This plugin provides tighter integration with [prisma-next](https://github.com/prisma/orm),
...
own combine slot so they don't collide. Prisma chooses the SQL execution
strategy; combining consumers does not inherently require multiple statements.
