...
declare global {
    export namespace PothosSchemaTypes {
        ...
        export interface SchemaBuilder<Types extends SchemaTypes> {
            ...
            loadableNodeRef: <Shape extends object, Key extends bigint | number | string, CacheKey = Key>(name: string, options: DataLoaderOptions<Types, Shape, Key, CacheKey> & LoadableNodeId<Types, Shape, Key>) => ImplementableLoadableNodeRef<Types, Key | Shape, Shape, Key, CacheKey>;
            ...
            loadableNode: "relay" extends PluginName ? <Shape extends NameOrRef extends ObjectParam<Types> ? ShapeFromTypeParam<Types, NameOrRef, false> : object, Key extends bigint | number | string, Interfaces extends InterfaceParam<Types>[], NameOrRef extends ObjectParam<Types> | string, CacheKey = Key>(nameOrRef: NameOrRef, options: LoadableNodeOptions<Types, Shape, Key, Interfaces, NameOrRef, CacheKey>) => Omit<ImplementableLoadableNodeRef<Types, Key | Shape, Shape, Key, CacheKey>, "implement"> : "@pothos/plugin-relay is required to use this method";
        }
