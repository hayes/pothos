...
describe('defer', () => {
        ...
        "data": {
          "post": {
            "comments": [
              ...
              {
                "author": {
                  "bio": "Thymum ait aggero ante vergo corroboro totidem cubo pariatur. Suggero sint coniuratio summopere deludo tendo arceo tenetur desidero. Terra dolor labore.",
                },
                "id": "2",
              },
              {
              ...
              {
                "author": {
                  "bio": "Curtus tandem ante harum careo vicinus annus utrum vacuus crinis. Sollicito ipsum sit crastinus compello adamo attero. Deleo arto celer earum conspergo ipsum virtus strues.",
                },
                "id": "4",
              },
              {
                "author": {
                  "bio": "Unde minus quae suspendo cubicularis deporto dedecor depopulo uterque. Consequuntur theologus aedificium ulciscor decipio tribuo vulticulus cruentus. Terreo aurum defetiscor adipisci cubitum assumenda.",
                },
                "id": "5",
              },
            ],
  ...
  it('defer type includes', async () => {
    ...
    expect(drizzleLogs).toMatchInlineSnapshot(`
      [
        "Query: select "d0"."id" as "postId", coalesce((select json_group_array(json_object('id', "id", 'text', "text", 'authorId', "authorId", 'postId', "postId", 'createdAt', "createdAt", 'updatedAt', "updatedAt", 'author', jsonb("author"))) as "r" from (select "d1"."id" as "id", "d1"."text" as "text", "d1"."author_id" as "authorId", "d1"."post_id" as "postId", "d1"."createdAt" as "createdAt", "d1"."createdAt" as "updatedAt", (select jsonb_object('firstName', "firstName", 'lastName', "lastName", 'id', "id", 'lowercaseFirstName', "lowercaseFirstName", 'profile', jsonb("profile")) as "r" from (select "d2"."first_name" as "firstName", "d2"."last_name" as "lastName", "d2"."id" as "id", (lower("d2"."first_name")) as "lowercaseFirstName", (select jsonb_object('id', "id", 'userId', "userId", 'bio', "bio") as "r" from (select "d3"."id" as "id", "d3"."user_id" as "userId", "d3"."bio" as "bio" from "profile" as "d3" where "d2"."id" = "d3"."user_id" limit ?) as "t") as "profile" from "users" as "d2" where "d1"."author_id" = "d2"."id" limit ?) as "t") as "author" from "comments" as "d1" where "d0"."id" = "d1"."post_id") as "t"), jsonb_array()) as "comments" from "posts" as "d0" where "d0"."id" = ? limit ? -- params: [1, 1, 1, 1]",
      ]
    `);
  });
});
