  ...
  it('batches when nested in lists', async () => {
    ...
    expect(drizzleLogs).toMatchInlineSnapshot(`
      [
        ...
        "Query: select "d0"."id" as "id", coalesce((select json_group_array(json_object('id', "id", 'comments', jsonb("comments"))) as "r" from (select "d1"."id" as "id", coalesce((select jsonb_group_array(json_object('id', "id", 'text', "text", 'authorId', "authorId", 'postId', "postId", 'createdAt', "createdAt", 'updatedAt', "updatedAt")) as "r" from (select "d2"."id" as "id", "d2"."text" as "text", "d2"."author_id" as "authorId", "d2"."post_id" as "postId", "d2"."createdAt" as "createdAt", "d2"."createdAt" as "updatedAt" from "comments" as "d2" where "d1"."id" = "d2"."id") as "t"), jsonb_array()) as "comments" from "posts" as "d1" where ("d1"."published" = ? and "d0"."id" = "d1"."author_id") order by "d1"."createdAt" desc limit ? offset ?) as "t"), jsonb_array()) as "posts" from "users" as "d0" where "d0"."id" in (?, ?, ?) -- params: [1, 1, 1, 10, 10, 10]",
      ]
    `);
