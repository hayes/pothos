...
Chained `.validate(schema)` calls receive omitted (`undefined`) and explicit `null` values.
Use a nullish schema when those inputs are allowed; a transform can supply a default, and the
resolver receives its inferred output. Validators passed in the `validate` option continue to
skip nullish values. Input type validators run only when an input object is present.
