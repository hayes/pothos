    ...
    expect(result).toMatchInlineSnapshot(`
      {
        ...
        "errors": [
          [GraphQLError: recursive.float: Invalid input, recursive.recurse.number: Number must be less than or equal to 5, contactInfo.aliases.0: Aliases should be capitalized, contactInfo.name: Name should be capitalized],
        ],
      }
    `);
  });
