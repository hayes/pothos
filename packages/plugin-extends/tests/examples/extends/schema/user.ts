    ...
    Query: (t) => ({
      user: t.field({
        ...
        resolve: (parent, args, { User }) => {
          const user = User.map.get(Number.parseInt(String(args.id), 10));
    ...
    Subscription: (t) => ({
      user: t.field({
        ...
        subscribe: async function* subscribe(parent, { id }, { User }) {
          yield await Promise.resolve(User.map.get(Number.parseInt(String(id), 10)));

          return User.map.get(Number.parseInt(String(id), 10));
        },
        resolve: (parent, args, { User }) => {
          const user = User.map.get(Number.parseInt(String(args.id), 10));
