...
const PostDraft = builder.prismaObject('Post', {
  ...
  fields: (t) => ({
    title: t.exposeString('title'),
    commentsSince: t.fieldWithSelection(
      (args) => ({
        comments: {
          where: {
            createdAt: {
...
`t.fieldWithSelection` method to select the appropriate data for this relationship.
...
The following example shows how we can hoist selections from a nested graphQL field into a parent
selection when it is not defined with `t.relation`
