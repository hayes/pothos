import type { LoadManyCallback, LoadOneCallback, Maybe } from 'grafast';
...
export interface NpcData {
  ...
  species: 'Changeling' | 'Rock Monster' | 'Half Elf' | 'Gondii' | 'Bopca Protector';
  name: string;
  exCrawler?: boolean;
  client?: number;
  clients?: number[];
  ...
  return {
    crawlers: [
      ...
      },
      {
        ...
        species: 'Human',
        name: 'Katia',
        items: ['Equipment:207', 'Consumable:203', 'Consumable:204', 'Equipment:210'],
    ...
    clubs: [
      ...
      },
      {
        ...
        stock: ['Consumable:203', 'Consumable:204', 'Consumable:209', 'UtilityItem:206'],
      },
      {
        id: 302,
...
export const batchGetCrawlersByIds: LoadManyCallback<
  number[],
  CrawlerData,
  Maybe<readonly Maybe<CrawlerData>[]>,
  never,
  Database
> = (idsList, { unary: data }) => {
  return idsList.map((ids) => ids.map((id) => data.crawlers.find((c) => c.id === id)));
};

export const batchGetNpcById: LoadOneCallback<number, NpcData, Maybe<NpcData>, never, Database> = (
  ids,
  { unary: data },
) => {
...
export const batchGetNpcsByIds: LoadManyCallback<
  number[],
  NpcData,
  Maybe<readonly Maybe<NpcData>[]>,
  never,
  Database
> = (idsList, { unary: data }) => {
  return idsList.map((ids) => ids.map((id) => data.npcs.find((c) => c.id === id)));
};
...
export const batchGetLocationsByFloorNumber: LoadManyCallback<
  number,
  LocationData,
  Maybe<readonly Maybe<LocationData>[]>,
  never,
  Database
