...
function makeBio(key: keyof typeof NAMES) {
...
`,
  ...
  {
    const stmt = db.prepare('INSERT INTO users (id, full_name, username, bio) VALUES (?, ?, ?, ?)');
    for (let nameIndex = 0; nameIndex < NAMES.length; nameIndex++) {
      stmt.run(nameIndex + 1, NAMES[nameIndex], NAMES[nameIndex].toLowerCase(), makeBio(nameIndex));
    }
    stmt.finalize();
  }

  {
    const stmt = db.prepare('INSERT INTO friendships (id, user_id, friend_id) VALUES (?, ?, ?)');
    let id = 0;
    for (let nameIndex = 0; nameIndex < NAMES.length; nameIndex++) {
      // A stable list of friendIds
      const friendIndexes = NAMES.map((_, idx) => idx).filter((idx) => idx % (nameIndex + 1) === 0);
