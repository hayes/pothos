...
Copy the interface you want into your namespace, delete its existing properties, and keep every generic exactly as declared (importing the generic types from `@pothos/core`). Add your own properties — make them optional (`newProp?: TypeOfProp`) unless the plugin genuinely requires them.
...
Runtime behaviour goes in the wrap hooks — `wrapResolve`, `wrapSubscribe`, and `wrapResolveType`. Each receives the function it wraps plus the field or type config, and returns either the original function or a replacement with the same signature:
...
<Callout type="info">
  See also: [SchemaBuilder](../fundamentals/schema-builder) for the `SchemaTypes` generic your plugin extends, the [errors plugin](./errors) for a plugin that adds field options and generated types, or the [relay plugin](./relay) for input wrapping in practice.
</Callout>
