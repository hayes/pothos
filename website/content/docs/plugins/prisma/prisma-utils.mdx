...
The publishing schema uses these helpers to expose a small, intentional input surface. A title
filter and ordering input support public post search:

<include cwd lang="typescript">local-examples/prisma/schema.ts#filters</include>
...
<include cwd lang="typescript">local-examples/prisma/schema.ts#mutations</include>
...
The [local setup](./setup#run-the-publishing-api) generates Prisma Utils types alongside the client
and runs these operations against SQLite. Scalar-list filters remain an alternative for databases
that support Prisma scalar lists.
