...
- **`findUniqueOrThrow` for non-null fields.** `team` and `myTeam` are non-nullable, so they must never resolve to `null`. `findUnique` returns `null` when nothing matches; `findUniqueOrThrow` throws instead. Reach for `findUnique` only when the field is marked `nullable`.
