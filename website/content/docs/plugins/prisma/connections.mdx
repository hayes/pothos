---
title: Connections
description: Build Relay connections over Prisma models with cursor pagination, total counts, and shared connection objects.
---
...
`t.relatedConnection` builds a connection from a relation of the current model, with no resolver needed since the relation names the data. It works on any Prisma object, and pairs naturally with a [node](./relay).
