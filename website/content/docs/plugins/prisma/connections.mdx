...
<Callout type="info">
  See also: [Relay nodes](./relay) for the `prismaNode` these connections hang off, [Relations](./relations) for `t.relation` and `relationCount`, or the [Relay plugin](../relay) for how connections, edges, and cursors work in general.
</Callout>
