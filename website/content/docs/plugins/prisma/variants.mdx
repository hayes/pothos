...
<Callout type="info">
  See also: [Interfaces](./interfaces) for sharing fields across variants of a model, [Relations](./relations) for the `query` and `type` options used above, or [Relay](./relay) for the `prismaNode` these examples build on.
</Callout>
