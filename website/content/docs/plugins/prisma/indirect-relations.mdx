...
<Callout type="info">
  See also: [Selections](./selections) for the `select` mode these examples use, [Relations](./relations) for the direct `t.relation` case, or [Type variants](./variants) for exposing the same model as more than one GraphQL type.
</Callout>
