...
or `{}` when nothing is included. The return type of a call with `select` is therefore
`{ select?, include? }`, the same as a call with neither. All of them spread into a prisma call,
and the given columns are on the rows either way.
