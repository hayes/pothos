...
spreads that narrow an interface or union to one of its implementations. When several
implementations share a field name and you need to target a specific one, a segment can be written
as `{ name, type }` to pin the type condition the field must be selected under:
