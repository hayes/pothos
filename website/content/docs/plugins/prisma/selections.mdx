...
<Callout type="info">
  See also: [Relations](./relations) for `t.relation` and relation counts, [Prisma objects](./objects) for `prismaObject` and `prismaField`, or [Indirect relations](./indirect-relations) for loading data across a join.
</Callout>
