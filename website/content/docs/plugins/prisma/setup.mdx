...
resolves after the initial payload, and loading what it needs up front would delay that payload for
data the client has already agreed to wait for. When the deferred fragment resolves, its fields are
loaded through [fallback queries](./relations#fallback-queries), batched as usual.
