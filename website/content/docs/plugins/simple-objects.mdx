...
Most object types need a backing model — the TypeScript shape your resolvers return. Usually you write that shape as an `interface` and hand it to `builder.objectRef`. The simple objects plugin skips that step: `builder.simpleObject` reads the fields you declare and infers the backing model from them, so a type and its shape are defined in one call. It's the right tool for response shapes that only exist to be returned — stats blocks, computed summaries, DTOs — where writing a separate interface is pure ceremony.
...
That trade is worth it when the type has no life outside the schema. A `TeamStats` block returned by one query doesn't need a hand-written interface — the fields already say everything. Reach for a plain `objectRef` when the backing model is a real domain entity you load, mutate, and pass around, where a named `interface` earns its keep.
...
The `fields` inside the second argument map one-to-one onto the inferred backing model — Pothos resolves each by reading the property of the same name off the parent. When a value isn't just passed down — it's derived, takes arguments, or hits another source — put it in the **third argument** instead. Those fields run through normal resolvers and receive the fully inferred parent, so `record` above can read `standing.stats.wins`.
...
Use whichever reads better: the third argument keeps a type's computed fields next to its declaration; a separate `builder.objectType` call is handy when the extra fields live in another module.
...
<Callout type="info">
  See also: [Object types](../fundamentals/objects) for the `objectRef` style `simpleObject` builds on, or [Interfaces](../fundamentals/interfaces) for `resolveType` and the interface fundamentals `simpleInterface` shares.
</Callout>
