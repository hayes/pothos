...
That trade is worth it when the type has no life outside the schema. A `TeamStats` block returned by one query doesn't need a hand-written interface; the fields already say everything. Use a plain `objectRef` when the backing model is a real domain entity you load, mutate, and pass around, where a named `interface` is worth defining.
