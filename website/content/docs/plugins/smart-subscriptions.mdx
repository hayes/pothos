...
A single field can register its own events through the same `subscribe` callback in the field options, which helps when one field has a narrower refresh trigger than its parent type:
