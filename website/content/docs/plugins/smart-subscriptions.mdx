...
The payoff is that you never decide up front which parts of the schema are subscribable — any type or field can register an event, so a live view of a game, its scores, or a single score line all come from the same query. Refetch options (below) narrow a refresh to just the sub-tree that changed, so a single score update doesn't re-run the whole query.
...
A single field can register its own events through the same `subscribe` callback in the field options — useful when one field has a narrower refresh trigger than its parent type:
...
<Callout type="info">
  See also: [Object types](../fundamentals/objects) for the `objectRef` these subscriptions hang off, or [Fields](../fundamentals/fields) for the field options that sit alongside `subscribe` and `canRefetch`.
</Callout>
