...
The publishing API uses the tables and builder from [Setup](./setup). Its public User describes
...
same object API. See [Relay](./relay) for that choice. Its public fields omit email and drafts;
those belong on the current author's [Viewer](./variants).
