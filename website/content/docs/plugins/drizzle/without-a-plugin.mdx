...
<Callout type="info">
  A [dataloader](../dataloader) is the other lever for N+1 — batch the per-player team lookups into one query. Or let the [Drizzle plugin](./setup) plan these selections for you: `t.relation` and `t.drizzleField` read the GraphQL selection set and build a single nested query. See also [Object types](../../fundamentals/objects) for the `objectRef` this page builds on.
</Callout>
