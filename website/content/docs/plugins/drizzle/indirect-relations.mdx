...
Drizzle's [relational query builder](https://rqbv2.drizzle-orm-fe.pages.dev/docs/relations-v2) can model a many-to-many directly. Point a relation's `from` and `to` at the join table with `.through()`, and the join disappears from the graph, so `players.games` reads as a plain list of games:
