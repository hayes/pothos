...
<Callout type="info">
  See also: [Connections](./connections) for the `t.relatedConnection` and `t.drizzleConnection` this page falls back from, [Relations](./relations) for the direct `t.relation` case, or [Type variants](./variants) for exposing the same table as more than one GraphQL type.
</Callout>
