...
<Callout type="info">
  See also: [Relations](./relations) for `t.relation` and relation counts, [Drizzle objects](./objects) for `drizzleObject` and `drizzleField`, or [Indirect relations](./indirect-relations) for loading data across a join.
</Callout>
