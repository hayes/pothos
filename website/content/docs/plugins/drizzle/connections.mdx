...
<include cwd lang="typescript" meta='playground example="plugin-drizzle" query="{ posts(first: 2) { nodes { title } pageInfo { endCursor hasNextPage } } }"'>playground-examples/plugin-drizzle/schema.ts#queries</include>
