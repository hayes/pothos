...
`t.relatedConnection` builds a connection from a relation of the current table — no resolver needed, since the relation names the data. It defines the `Connection` and `Edge` types for you and pairs naturally with a [node](./relay).
...
<Callout type="info">
  See also: [Indirect relations](./indirect-relations) for paginating through a join table with `drizzleConnectionHelpers`, [Relay nodes](./relay) for the `drizzleNode` these connections hang off, or the [Relay plugin](../relay) for how connections, edges, and cursors work in general.
</Callout>
