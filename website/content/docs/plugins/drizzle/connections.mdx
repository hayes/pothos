...
That includes a `where` on the relation, and the junction table of a many-to-many relation defined
with `.through(...)`. The count joins the junction table the same way the rows do, so a row that
matches the junction twice counts twice, and appears twice in the connection.
