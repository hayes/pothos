...
The [publishing playground](/playground?example=plugin-drizzle) runs real SQLite in WebAssembly,
using Drizzle's SQLite proxy driver to execute statements in the browser. Its `tables.ts` defines
the tables and relations; `database.ts` initializes deterministic data; `schema.ts` defines the
GraphQL schema. No database credentials or external database service are needed. The WebAssembly
module is downloaded when the example loads.

The browser bridge runs each SQL statement in its own connection and does not implement
transactions spanning several statements. The database belongs to that schema instance. Reloading or rebuilding the source creates fresh
state. This is an example runtime; use the driver and migration workflow for your application's
database, such as the libSQL setup above.
