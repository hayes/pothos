...
When `client` is a function, `relations` is required in the config — the plugin can no longer read it off a fixed client.
...
<Callout type="info">
  See also: [Drizzle objects](./objects) to define your first type, [Relations](./relations) to connect them, or the [Drizzle plugin overview](../drizzle) for the full picture.
</Callout>
