...
Throwing is the type-safe path: a thrown value is untyped, so the resolver's return type stays `Team`. The plugin also unwraps errors you *return* rather than throw, but returning an `Error` widens the resolver's return type, so prefer `throw` for fields, and use the return form with [`errorUnion`](#manual-error-unions) and [`errorUnionField`](#custom-error-unions) below, which type the errors into the signature.
...
`itemErrors` also works with sync and async iterators, which is useful with `graphql@>=17` or any executor that supports the `@stream` directive. A yielded error becomes an error item; if the generator itself throws, that error is added as the final item and no further results are produced for the field:
...
`builder.errorUnion` builds a reusable union type up front. Any field that returns it automatically handles both returned and thrown errors, which helps when several fields share the same success-or-error shape.
