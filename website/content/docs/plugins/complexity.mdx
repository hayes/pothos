...
The defaults treat every field the same, but real fields have real costs. Set a `complexity` option on any field to override its baseline. It takes three forms:
...
<Callout type="info">
  See also: [Fields](../fundamentals/fields) for the field options `complexity` sits beside, [Object types](../fundamentals/objects) for the `objectRef` the `Team` and `Player` types build on, or the [dataloader plugin](./dataloader) for taming the n+1 fan-out that list multipliers stand in for.
</Callout>
