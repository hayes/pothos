...
Authorization in Pothos lives on the schema, not scattered through resolvers. The scope-auth plugin lets you name the checks your app cares about — "is this a member," "is this staff," "can this user manage that team" — and attach them to any field or type with an `authScopes` option. Checks run before the resolver, are cached per request, and compose with `$any`/`$all`.
...
Open the example and empty the **Context** tab: `wins` and `scoutingReport` now fail while `id` and `name` still resolve. The plugin is general-purpose — nothing here assumes a particular auth philosophy, so the same machinery drives role-based, permission-based, or ownership-based schemes.
...
const builder = new SchemaBuilder<{
  Context: { viewer?: { id: string; role: 'member' | 'staff' } };
...
authScopes: async (context) => ({
  ...
  canManageTeam: (teamId: string) =>
    context.permissions.canManage(context.viewer, teamId),
});
```
...
authScopes: async (context) => ({
  ...
  canManageTeam: context.viewer
    ? (teamId: string) => context.permissions.canManage(context.viewer!, teamId)
    : false,
});
```

You can also defer a plain boolean by wrapping it in a function (`() => context.viewer.isStaff()`) so it's evaluated on first use rather than during initialization.
  ...
  fields: (t) => ({
    scoutingReport: t.exposeString('scoutingReport', {
      authScopes: (team, _args, context) => {
        ...
        if (context.viewer?.id === team.coachId) {
          return true;
        }
...
<Callout type="warn">
  Setting scopes based on a field's *return* value isn't supported directly — the check runs before the resolver. Move the check onto the returned type instead, and reach for [`runScopesOnType`](#running-scopes-on-the-type) so it fires once per object.
</Callout>
...
const builder = new SchemaBuilder<{
  ...
  scopeAuth: {
    ...
    authScopes: async (context) => ({ member: !!context.viewer }),
  },
});
...
A scope often narrows what you know about the context — once `member` passes, `viewer` is non-null. Declare that refinement with the `AuthContexts` type and read it through `t.authField`:

```typescript
const builder = new SchemaBuilder<{
  Context: { viewer?: { id: string } };
  AuthScopes: { member: boolean };
  AuthContexts: { member: { viewer: { id: string } } };
}>({
  plugins: [ScopeAuthPlugin],
  scopeAuth: {
    authScopes: async (context) => ({ member: !!context.viewer }),
...
builder.queryField('viewerId', (t) =>
  t.authField({
    type: 'ID',
    authScopes: { member: true },
    // context.viewer is non-null here.
    resolve: (_parent, _args, context) => context.viewer.id,
  }),
);
```
...
builder.queryField('me', (t) =>
  t.withAuth({ member: true }).prismaField({
    ...
    resolve: (query, _root, _args, context) =>
      prisma.user.findUniqueOrThrow({ ...query, where: { id: context.viewer.id } }),
  }),
);
```
...
`JSON.stringify` handles most objects; for circular references or key-order stability, reach for something like `faster-stable-stringify`.
...
Pass `disableScopeAuth` to `toSchema` to build the schema with every check turned off — handy for tests that shouldn't thread auth context through every query:
...
<Callout type="info">
  See also: [Handling errors](../patterns/handling-errors) for how thrown and masked errors reach the client, or [Objects](../fundamentals/objects) for the `objectRef` the guarded types are built from.
</Callout>
