...
To change how the `node`/`nodes` query fields load, pass a `resolve` in `nodeQueryOptions` / `nodesQueryOptions`. Each receives a `resolveNode`/`resolveNodes` callback for the default behaviour:
