...
<Callout type="info">
  The builder key is `relay`. Older versions used `relayOptions`; that name is only accepted in v3-compatibility mode — new code should use `relay`.
</Callout>
...
- `loadWithoutCache` / `loadManyWithoutCache` — skip the cache. Reach for these if caching is undesirable or you already load through a dataloader.
...
When the type parameter is a class, `isTypeOf` defaults to an `instanceof` check (falling back to the prototype's constructor), so class-backed nodes often need nothing here — but declaring it explicitly is clearer.
...
Restrict which node types an argument accepts with `for` — a single ref or an array:
...
`t.node` and `t.nodes` add standalone node fields anywhere. Their `id`/`ids` return values match `t.globalID` — a global ID string or an `{ id, type }` object. Loading goes through the same per-request cache, so a node used in several places loads once.
...
`t.globalConnectionField` and `t.globalConnectionFields` add a field to *every* `Connection` type — useful for a `totalCount`:
...
<Callout type="info">
  See also: [Interfaces](../fundamentals/interfaces) for how the `Node` interface and `resolveType` work, or [`plugin-dataloader`](./dataloader) for batching the loaders behind your nodes.
</Callout>
