...
Directives predate code-first schemas and are really an SDL-first idea. Reach for this plugin when you want to keep using a directive-based tool — rate limiting, auth, caching — from a Pothos schema.
...
The plugin writes the directive data onto the `extensions` of the underlying GraphQL type; it doesn't enforce the directive itself. A downstream transformer reads those extensions and does the real work.
