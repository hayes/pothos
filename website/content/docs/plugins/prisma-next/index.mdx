...
<Callout type="warn">
  Experimental. This plugin tracks prisma-next `^0.14.0` (the
  `@prisma-next/*` client packages, published on npm). Both prisma-next and
  this plugin are pre-1.0 and their APIs may still change. Pending a stable
  npm release, the plugin is available as a PR preview via
