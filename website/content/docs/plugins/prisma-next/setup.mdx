...
The plugin lists the prisma-next client as a peer dependency, so install the
`@prisma-next/*` packages too (and a runtime adapter for your database — e.g.
`@prisma-next/sqlite`). Pin them to the same `^0.14.0` range the plugin
targets:

```package-install
npm install --save @prisma-next/contract @prisma-next/sql-contract @prisma-next/sql-orm-client @prisma-next/sqlite
```
