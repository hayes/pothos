...
The input argument is required by default. Set `argOptions.required: false` to make the whole argument optional, which suits a search or filter field that runs with no input at all:
