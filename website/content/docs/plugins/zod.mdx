...
<include cwd lang="typescript" meta='playground example="plugin-zod"'>playground-examples/plugin-zod/schema.ts#roster</include>

Change `maxLength` from `2` to `1`, rebuild, and run the valid operation again: it now fails the list
constraint. This example is for maintaining existing schemas; choose the [validation plugin](./validation)
