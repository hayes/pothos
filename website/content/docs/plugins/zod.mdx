...
<Callout type="warn">
  ...
  plugin keeps working — reach for it only on schemas already built around it.
</Callout>
...
The remaining keys depend on the field's type. Each value is a `Constraint` — a bare value, or `[value, { message }]`:
...
<Callout type="info">
  See also: the [validation plugin](./validation) for the recommended, library-agnostic successor, [Fields](../fundamentals/fields) for the arguments these validators guard, or [Handling errors](../patterns/handling-errors) for turning a failed validation into a typed result.
</Callout>
