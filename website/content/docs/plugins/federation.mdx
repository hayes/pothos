...
description: Build Apollo Federation 2 subgraphs — define entities, extend external types, and print a composable subgraph schema.
---
...
| `listSize` | `@listSize` — `{ assumedSize?, slicingArguments?, sizedFields?, requireOneSlicingArgument? }` | fields |
...
<Callout type="info">
  See also: [Object types](../fundamentals/objects) for the `objectRef` entities build on, [Interfaces](../fundamentals/interfaces) for the interface entities above, or the [directives plugin](./directives) that powers every federation directive.
</Callout>
