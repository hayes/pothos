...
Nothing is appended when Pothos cannot find a key it can rely on. That covers tables with no primary
key or unique column, and composite keys declared with `primaryKey({ columns: [...] })` whose columns
are not individually marked `notNull()`. Those connections keep the ordering you wrote, so give them
a tie breaker yourself.
