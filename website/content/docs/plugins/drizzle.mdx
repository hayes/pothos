...
Orderings that already contain the primary key, or any unique column that is not nullable, are used
as provided. Nullable unique columns are not treated as unique, because rows containing a `null` are
still tied with each other.
...
`orderBy` accepts the name of any extra selected by the same query. Pothos orders by the expression,
