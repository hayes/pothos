...
A cursor names a row's position in the connection's ordering. That works only if the ordering can't
put two rows in the same position, and if the values the cursor carries still mean the same thing
when they come back. This applies to every connection in this plugin: `t.relatedConnection`,
`t.drizzleConnection`, and `drizzleConnectionHelpers`.

### The ordering has to be unique

`orderBy: { createdAt: 'desc' }` leaves rows sharing a timestamp in an order the database is free to
change between queries, so paging can return a row twice or skip it. Pothos appends the primary key
when the ordering doesn't already cover a unique, non-nullable set of columns, so this:

```ts
orderBy: { createdAt: 'desc' }
```

is queried as `createdAt desc, id desc`. Write the tie breaker yourself if you want it in a different
...
Orderings that already end in a primary key, or in any unique non-nullable column, are left alone.
A nullable unique column doesn't count: rows sharing a `null` still tie with each other.

### Cursor values have to round trip

A cursor stores the value your driver handed to JavaScript, then compares that value against the
column on the next request. If the two aren't the same value, the comparison is wrong.

Timestamps are the case that bites. `timestamp({ mode: 'date' })` gives you a JavaScript `Date`,
which holds milliseconds, while a Postgres `timestamptz` stores microseconds. A row stored at
`.086068` produces a cursor holding `.086`, and `created_at < '.086'` matches nothing else in that
millisecond, so those rows never appear on any page.

There are two ways to fix it.

**Keep the full value in JavaScript.** `mode: 'string'` hands you the timestamp as text with the
microseconds intact:
...
Nothing else changes. The cursor holds the text, Postgres parses it back to the exact value it
stored, and the column is still what gets ordered and compared, so an index on it is still used. The
field is a `string` in your code rather than a `Date`.

**Order by an expression instead.** To keep the `Date` mapping, select the full value as an extra and
order by that:
...
`orderBy` accepts the name of any extra the same query selects. Pothos orders by the expression,
...
Write extras as a callback. Drizzle aliases the table it queries and passes that alias to the
callback; an expression built from the imported table object refers to a name the query doesn't have.

Two things to get right:

- The expression has to sort the way its values compare. A timestamp rendered as fixed-width UTC text
  sorts the same way it does chronologically. A local-time or variable-width format doesn't, and will
  skip rows.
- Ordering by an expression can't use an index on the underlying column. Add an index on the
  expression if the table is large enough to need one.
