...
related rows: a row reachable through two junction rows counts once. A `t.relatedConnection`'s
`totalCount` counts the rows the connection pages over instead, which is one per junction row,
because that is what the relational query builder returns for the relation.
...
Whatever the relation itself declares always applies to the count, whether or not the filter does:
a `where` on the relation, and the junction table of a many-to-many relation defined with
`.through(...)`, which the count joins in the same way the rows do (so a row that matches the
junction twice counts twice, as it also appears twice in the connection).
