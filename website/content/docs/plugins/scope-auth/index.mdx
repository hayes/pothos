---
title: Scope auth plugin
description: Authorize fields and types with request scopes, permission loaders, and ownership checks.
---
...
For schemas using Relay, see [Relay node authorization](/docs/plugins/scope-auth/relay-nodes)
...
```typescript playground example="plugin-scope-auth"
builder.queryType({
  fields: (t) => ({
    message: t.string({
      authScopes: {
...
builder uses the client `prisma` and generated types from [Prisma setup](/docs/plugins/prisma/setup), with a
...
`skipInterfaceScopes` on an Object type skips the interfaces it implements when resolving its own
fields, but does not skip the scopes of the interface that declared an inherited field. Use
`skipTypeScopes` or `skipInterfaceScopes` on the interface field itself for that.
