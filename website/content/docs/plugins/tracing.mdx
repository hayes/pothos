...
`tracing.wrap` turns the tracing decision into behaviour. It runs with three arguments and returns the resolver the schema will actually call:
...
<Callout type="info">
  See also: [SchemaBuilder](../fundamentals/schema-builder) for the builder options every plugin extends, [Fields](../fundamentals/fields) for the field options `tracing` sits alongside, or [First server](../getting-started/first-server) for the graphql-yoga wiring the execution-phase examples hook into.
</Callout>
