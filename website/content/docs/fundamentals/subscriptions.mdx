---
title: Subscriptions
description: The Subscription root type, and fields that push a stream of values to clients over a long-lived connection.
---
...
Declaring `pubSub` on the builder's `Context` type is what makes `ctx.pubSub` typed in every resolver, like the rest of [context](./context).
...
The `ctx.user` guard is covered in [Context](./context#requiring-a-signed-in-user). Checks that depend on the individual event go in `resolve` instead.
