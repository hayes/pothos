...
`builder.interfaceRef<ICharacter>('Character')` names the interface and states the backing model behind it, the way `objectRef` does for an object type (the [Object types](./objects) guide covers references and backing models). Here the backing model is `ICharacter`, a union of the shapes that implement the interface; each of them extends `ICharacterBase`, which carries the shared `id` and `name`. `builder.interfaceType` defines the interface's fields with the same field builder object types use, so `t.exposeID('id')` and `t.exposeString('name')` read those two properties off the backing model, and every implementing type inherits them. The `resolveType` option is covered under [Resolving the concrete type](#resolving-the-concrete-type).
