...
The tabs above each define the same `Character` object type. The rest of this page uses `objectRef`, shown first: `builder.objectRef<ICharacter>('Character')` names the type and states the TypeScript shape of the data behind it (Pothos calls this the backing model), and `.implement` supplies the type's fields. `t.exposeID('id')` and `t.exposeString('name')` return those properties straight from that data; the [Fields](./fields) guide covers the field builder in detail. The **Classes** and **Builder types** tabs define the same type from a class and from a registered name, [covered below](#definition-styles).
...
`builder.objectType` takes a reference in each of these forms as its first argument: a class, an existing object ref, or a string name registered on the builder's `Objects` generic. `implement` is the object-ref shorthand: calling `.implement(options)` on a ref is the same as passing that ref and the options to `builder.objectType`. The three forms are all first-class; [Definition styles](#definition-styles) covers when each one fits.
...
Wrapping the reference in an array — `type: [Character]` — makes the field a list; a bare `type: Character` returns a single one. Because the field returns `Character`, its resolver has to return values matching the backing model, and TypeScript reports an error if it doesn't. The [Queries](./queries) guide covers the `Query` root itself.
