...
The companion query reads `user.id` from **Context**. Change that value and run again. Adapt the named interface and query into a small reproduction of your own issue. The playground uses strict TypeScript settings; it does not reproduce your application's package installation or editor configuration.
