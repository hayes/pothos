...
For a smaller example of mapping data to a different API shape, try
[converting a stored height in meters to feet](/playground?example=foundations-overview).
...
You can run this schema immediately with a Playground action above. To run it in your own
application, add the server below.
