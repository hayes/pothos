...
to add in our schema needs to be explicitly defined. The `fields` property in the options object
should be a function that accepts one argument (a [FieldBuilder](../api/field-builder)) and returns
an object whose keys are the field names, and whose values are `FieldRefs` created by the
