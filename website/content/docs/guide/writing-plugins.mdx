...
that apply type-level behavior should use `parentType` to look up the owner. Before core 4.15,
`parentType` named the declaring interface even when building an inherited object field; plugins
that rely on owner-specific configs should require core 4.15 or later.
