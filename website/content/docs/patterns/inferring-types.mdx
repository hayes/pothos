...
`RaceShape` is `IRace`. `RaceFilterShape` is `{ nameContains?: string | null; immortal?: boolean | null }`. The pattern is similar to Drizzle's `InferSelectModel` and `InferInsertModel`.
