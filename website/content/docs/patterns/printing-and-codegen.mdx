...
`printSchema` from `graphql` serialises a `GraphQLSchema` to SDL. `lexicographicSortSchema` sorts types and fields alphabetically — without it the output order changes whenever the source order changes, which makes diffs noisy.
...
```typescript playground example="patterns-printing-schema"
