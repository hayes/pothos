...
Defines an object type. `objectRef` is the pattern to reach for first — see [Object types](../fundamentals/objects) for when to prefer it over a class or a `SchemaTypes`-registered name.
...
- `fields`: a function that receives a [`FieldBuilder`](./field-builder) and returns a map of field names to field refs.
- `isTypeOf`: recommended when implementing interfaces. Pothos calls it to decide whether a value of an implemented interface is of this type.
...
<Callout type="info">
  See also: [SchemaBuilder](../fundamentals/schema-builder) for configuring the generic and constructor, [FieldBuilder](./field-builder) for what goes inside a `fields` function, or [Object types](../fundamentals/objects) for the `objectRef`-first workflow.
</Callout>
