...
To install the prerelease version of Pothos, you can use the `next` tag:

```package-install
npm install --save @pothos/core@next
npm install --save @pothos/plugin-relay@next @pothos/plugin-prisma@next
```

Be sure to install the prerelease version (using the `next` tag) of any other plugins you are using
as well.
...
the `InferredResolveOptions` interface to allow plugins to replace or change the resolve functions
types globally.
...
The `InferredResolveOptionsByKind` interface can be used to get the `resolve` option by default, but
