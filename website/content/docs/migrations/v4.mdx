...
const builder = new SchemaBuilder<{
  ...
  relay: {
    ...
    // but will be overwritten by `cursorType` so you also need to explicitly set them
    edgeCursorType: 'String',
    pageInfoCursorType: 'String',
    // If you are using the new v4 nullability defaults, you may need to change the nullability of mutation fields
...
can't easily be made backward-compatible with the 3.x releases. Below is a summary of the main
