...
function resolveImportPath(
  ...
  // Index files are registered under their directory name (see the
  // `.replace(/\/index$/, '')` in `readDtsFiles`), so an explicit
  // `./types/global/index.js` import must resolve to
  // `@pothos/core/types/global`, not `@pothos/core/types/global/index`.
  if (resultParts.length > 2 && resultParts[resultParts.length - 1] === 'index') {
    resultParts.pop();
  }
