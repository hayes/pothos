...
it('bundles nested files and preserves variant inputs on inherited operations', async () => {
  ...
  try {
    ...
    await writeFile(join(example, 'metadata.json'), JSON.stringify({ id: 'greeting', title: 'Greeting', variants: [{ id: 'english', title: 'English', default: true }, { id: 'french', title: 'French' }] }));
    ...
    execFileSync(process.execPath, [fileURLToPath(new URL('../../../node_modules/tsx/dist/cli.mjs', import.meta.url)), fileURLToPath(new URL('../build-playground-examples.ts', import.meta.url))], { cwd: root, stdio: 'pipe' });
    const base = JSON.parse(await readFile(join(root, 'public/playground-examples/greeting.json'), 'utf8'));
    const variant = JSON.parse(await readFile(join(root, 'public/playground-examples/greeting-variant-french.json'), 'utf8'));
    expect(base.files.map((file: { filename: string }) => file.filename)).toEqual(['schema.ts', 'models/schema.ts']);
