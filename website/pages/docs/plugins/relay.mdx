...
number or string, which will be converted to a globalID. The `loadOne`, `loadMany`
`loadWithoutCache` methods are optional, with `loadMany` taking precidence. Generally you should
only implement ONE of these methods.These methods allow a nodes to be loaded by id. The relay plugin
adds to new query fields `node` and `nodes` which can be used to directly fetch nodes using global
IDs. Each node will only be loaded once by id, and cached if the same node is loaded multiple times.
`loadWithoutCache` can be used if caching is not desired, or the entity is already being loaded from
something like a dataloader that does it's own caching.
