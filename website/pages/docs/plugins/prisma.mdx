...
In some cases, you may want to craete multiple graphql types for a single prisma model. Using
`builder.prismaObject` multiple times with the same model will result in an error, because Pothos
will not know which version to use when creating relations.

To create additional types based on the same model you can add a `variant` option to your
`prismaObject` (or `prismaNode`) call. To use this type you will need to use the returned
`ObjectRef` when defining other fields.
...
// Use the type with a prismaField
builder.queryField('viewer', t => t.prismaField({
  // specify type with the ref rather than the model name as a string
  type: Viewer,
  resolve: (query, _root, args, { user }) => prisma.user.findUnique({ ...query, where: { id: user.id }})
}))

// Using the type with a relation field
builder.prismaObject('Post', {
  findUnique: post => ({ id: post.id }),
  fields: (t) => ({
    id: t.exposeID('id'),
    author: t.relation('author')
    // Pass ref as Type to use the variant instead of the primary type
    authorAsViewer: t.relation('author', { type: Viewer }),
  }),
});
```

You may also want to reference a variant as a child of the primary type. To do this, you can use the
`t.varient` field builder method. This will create a field that resolves to the same record, as the
variant graphql type. Any child selections on the variant field will be included as part of the
optimized prisma query, so relationships inside the variant will still have n+1 optimizations
applied, unless there are incompatible includes or selections, which would automatically be split
into a second query.
...
const Viewer = builder.prismaObject('User', {
  ...
  fields: (t) => ({
    id: t.exposeID('id'),
    // You can reference the primary type using the model name as a string
    user: t.variant('User'),
  }),
});
...
builder.prismaObject('User', {
  ...
  fields: (t) => ({
    // To reference a varient, pass the variants ref to `t.variant`.
    viewer: t.variant(Viewer, {}),
  }),
});
```

The above options will also work the the relay equivelents (`builder.prismaNode`,
`t.relatedConnection`, etc)
