...
Creating connections from indirect relations is a little more involved, but can be done by manually
creating a PrismaConnection object
...
const MediaConnection = builder.prismaConnectionObject(
  {
    type: Media,
    cursor: 'id',
    // additional options like name, description, etc
  },
  {
...
You may run into circualr reference issues if you use 2 prisma object refs to reference eachother.
