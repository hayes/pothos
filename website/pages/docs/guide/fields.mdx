---
name: Fields
title: Defining fields in Pothos GraphQL
menu: Guide
description: Guide for defining filds in Pothos
---
...
function. This is a function that takes a [FieldBuilder](../api/field-builder) as an argument,
and returns an object who's keys are field names, and who's values are fields created by the
[FieldBuilder](../api/field-builder). These examples will mostly add fields to the `Query` type,
...
not be added there like [Unions](./unions) and [Enums](./enums), you can use a `Ref` returned by
the builder method that created them in the `type` parameter. For types created using a class
\([Objects](./enums) or [Interfaces](./interfaces)\) or [Enums](./enums) created using a
