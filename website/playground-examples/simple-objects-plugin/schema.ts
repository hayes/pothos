...
const Comment = builder.simpleObject(
  ...
  (t) => ({
    preview: t.string({
      resolve: (comment) => {
        return comment.text.length > 50 ? `${comment.text.substring(0, 50)}...` : comment.text;
      },
    }),
  }),
);
  ...
  fields: (t) => ({
    ...
    post: t.field({
      ...
      resolve: (_, args) => ({
        ...
        content:
          'The Simple Objects plugin makes it easy to define GraphQL types without needing separate TypeScript type definitions. This reduces boilerplate while maintaining full type safety.',
        published: true,
        authorId: '1',
      }),
    }),
