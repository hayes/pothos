...
// for the 100+ raw .d.ts file libs so resetMonacoSetup can release
// them. Idempotent: subsequent calls noop until reset.
let prismaNextDisposables: MonacoDisposable[] | null = null;

/**
 * Drop the bundled @prisma-next/* .d.ts files into Monaco as
 ...
 * (generated by scripts/bundle-types.ts from each package's exports
 * field) is the load-bearing piece — the extra-libs are there for
 * chunked content-hash files that the entry `.d.ts` files import via
 * relative paths.
 *
