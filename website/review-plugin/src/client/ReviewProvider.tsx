...
export function ReviewProvider({
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: pathname re-resolves the content root after client navigation
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: pathname redraws highlights after client navigation
  useEffect(() => {
  ...
  useEffect(() => {
    if (!enabled || mode !== 'select' || !contentRoot) {
      return;
    }
    const handler = () => {
  ...
  useEffect(() => {
    if (!enabled || mode !== 'element' || !contentRoot) {
      return;
    }
    let lastHover: HTMLElement | null = null;
    const clearHover = () => {
      if (lastHover) {
        lastHover.classList.remove('rp-target-hover');
      }
      lastHover = null;
    };
  ...
  const handleReply = useCallback(
    (id: string, body: string) => review.reply(id, body).then(() => undefined),
    [review],
  );
  ...
  const openCount = review.all.filter(
    (c) => c.status === 'open' || c.status === 'needs-clarification',
  ).length;
  ...
  return (
    <div className="rp-root">
      ...
      {enabled && panelOpen && (
        // biome-ignore lint/a11y/noStaticElementInteractions: panel only stops clicks bubbling to the page
        // biome-ignore lint/a11y/useKeyWithClickEvents: panel only stops clicks bubbling to the page
