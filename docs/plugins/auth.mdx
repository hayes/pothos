...
interfaces or unions that resolve to this type, sine there is no way to determine the resolved type
of union or interface before it has neen resolved.

If you are using interfaces or unions, you can use a `postResolveCheck` instead. `postResolveCheck`s
will run for each instance of a type after it has been resolved, and will work with interface and
unions fields as well.
