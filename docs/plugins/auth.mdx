...
check for the children. To address these kinds of use cases, the auth plugin allows on a field to
return a set of checks the child should be authorized for.
...
As your schema gets more complicated, some types may be refferenced by fields on a lot of different
types. This can make it hard to keep all the auth checks for this popular type in sync and ensure
that all fields have appropriate auth checks. To make this simpler there is a preResoleAuthCheck you
can add to object types that allows you to define a check that will run before any resolver for that
type.
