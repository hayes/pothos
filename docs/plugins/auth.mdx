...
`permissionCheck` on any field on that type. permissions defined in the `permissions` do _NOT_ work
...
One caviot to note about `preResolveCheck` is that it will _NOT_ run before fields that return
interfaces this type implemnts, since those fields may resolve to another type instead. The current
workaround for this is to not depend on the `preResolveCheck` granting permissions when implementing
interfaces. This can be done in one of 3 ways:

- Not grantinig permissions from the `preResolveCheck`
- defining any granted permissions in the `permissions` option as a fallback
- (less ideal) granting the same permissions (when appropriate) from fields that return the
  interface.

Eventually there may be a better solution for this use case.
