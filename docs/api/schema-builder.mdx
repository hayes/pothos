...
## `mutationType(name, options)`

## `subscriptionType(name, options)`
...
- `interfaces`: an array of interfaces implemented by this object type. Items in this array can
  either be a key of the `Interface` property in [`TypeInfo`], or be an Interface type create by
  [`interfaceType`]. The backing model for the object type must `extend` the backing model of all
...
- `description`: A description of the current type
- `types`: an array of objects included in the union type. Items in this array can either be a key
  of the `Object` property in [`TypeInfo`], or be an Object type create by [`objectType`].
