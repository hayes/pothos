...
A Type Parameter for a Field can be any `TypeRef` returned by one of the [`SchemaBuilder`] methods
for defining a type, a class used to create an object or interface type, a ts enum used to define a
graphql enum type, or a string that corresponds to one of they keys of the `Objects`, `Interfaces`,
or `Scalars` objects defined in [`TypeInfo`].
