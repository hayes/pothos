...
easy, once you understand the core concepts of how GiraphQls type system works. Don't worry if the
...
3. The name key/name for the plugin in the `Plugins` interface i `global-types.ts`
...
Adding new method to SchemaBuilder or one of the FieldBuilder classes is also done through extending
interfaces. Extending these interfaces is how typescript is able to know these methods exist, even
though they are not defined on the original classes.
