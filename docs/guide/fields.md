...
function. This is a function that takes a [FieldBuilder](../field-builder.md) as an argument, and
returns an object who's keys are field names, and who's values are fields created by the
[FieldBuilder](../field-builder.md). These examples will mostly add fields to the `Query` type, but
...
be added there like [Unions](unions.md) and [Enums](enums.md), you can use a `Ref` returned by the
