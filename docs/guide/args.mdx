...
Similar to the [Fields Guide](/docs-guide-fields), the examples here will mostly be for the Query
type, but the same patters can be used anywhere that arguments for fields can be defined, including
both Object and Interface types.
...
2. Using the generic `t.arg.type` method
...
3. Using the `arg` method (alias for `t.arg.type`)
...
Args of non-scalar types can also be created with the `t.arg` or `t.arg.type` methods.
...
[inputs]: /docs-guide-inputs
[enums]: /docs-guide-enums
