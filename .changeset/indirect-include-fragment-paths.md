...
- When several implementations share a field name, matches whose field returns a different model or table than the one the query is being built for are ignored, so their selections are never merged into the wrong query.
